...
bool scanner::read_v_mem( PVOID x )
{
		...
		for ( size_t x = 0; x < this->_mbi.RegionSize / 2; ++x )
		{
			for ( size_t y = 0; y < ___length; ++y )
			{
				...
				if ( y == ___length - 1 )
				{
					...
					_log( LDEBUG, "r -> 0x%x", _address + x * 2 );
				}
			}
		}
	}
...
std::vector<size_t> scanner::scan_mltbyte( std::string string )
{
	size_t ___length = string.length( ); std::vector<size_t> _mem_locations;
	...
	for ( this->_address = 0; this->query_memory( ); this->_address += this->_mbi.RegionSize )
	{
		...
		for ( size_t x = 0; x < this->_mbi.RegionSize; ++x )
		{
			for ( size_t y = 0; y < ___length; ++y )
			{
				if ( buffer[ x + y ] != string[ y ] ) break;

				if ( y == ___length - 1 )
				{
					_mem_locations.push_back( this->_address + x );
					_log( LDEBUG, "r -> 0x%x", _address + x );
				}
			}
		}
	}
...
void scanner::rewrite_unicode( size_t addrss, std::string str )
{
	for ( size_t x = 0; x < str.length( ); ++x )
	{
		ZwWriteVirtualMemory( this->p_handle, ( LPVOID ) ( addrss + x * 2 ), &str[ x ], 1, nullptr );
	}

	_log( LDEBUG, "wu -> 0x%x", addrss );
}


void scanner::rewrite_mltbyte( size_t addrss, std::string str )
{
	for ( size_t x = 0; x < str.length( ); ++x )
	{
		ZwWriteVirtualMemory( this->p_handle, ( LPVOID ) ( addrss + x ), &str[ x ], 1, nullptr );
	}

	_log( LDEBUG, "wm -> 0x%x", addrss );
}